...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('repository');p.add_argument('--max-bytes',type=int,default=80000);p.add_argument('--max-files',type=int)
    ...
    result,sources=analyze(repo,args.report,args.max_files);packet,path=build_packet(repo,result,sources,args.max_bytes)
